...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer("ഇത് നിങ്ങളുടേതല്ല🤔", show_alert=True)
        return
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    await query.message.edit_text("ഇത് നിങ്ങളുടേതല്ല🤔", quote=True)
        ...
        if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
            ...
            await query.answer("Make sure I'm present in your group!!", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        if not files_:
            return await query.answer( ' where is file 😬. ')
        files = files_[0]
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('🖤 ഞാൻ നിങ്ങളുടെ private ചാറ്റിൽ അയച്ചിട്ടുണ്ട് (𝙇𝙊𝙊𝙆 𝙔𝙊𝙐𝙍 𝙋𝙈)❤', show_alert=True)
